...
######-------check r_Fail
POI="r_PassBin0m,r_PassBin1m,r_PassBin2m,r_FailBin0m,r_FailBin1m,r_FailBin2m"
...
#combine -M MultiDimFit -d ${1} -m 1000 --algo singles --cl=0.68 --redefineSignalPOIs ${POI} -n scan --freezeParameters rgx{CMS.*},rgx{PS.*},rgx{.*ACCEPT},rgx{.*norm},rgx{lumi.*},rgx{prop_bin.*} 
#combine -M MultiDimFit -d ${1} -m 1000 --algo singles --cl=0.68 --redefineSignalPOIs ${POI} -n scan --freezeParameters rgx{CMS.*},rgx{PS.*},rgx{.*ACCEPT},rgx{.*norm},rgx{lumi.*}
#combine -M MultiDimFit -d ${1} -m 1000 --algo singles --cl=0.68 --redefineSignalPOIs ${POI} -n scan --freezeParameters rgx{CMS.*},rgx{PS.*},rgx{.*ACCEPT},rgx{.*norm},rgx{prop_bin.*} --verbose 3
#combine -M MultiDimFit -d ${1} -m 1000 --algo singles --cl=0.68 --redefineSignalPOIs ${POI} -n scan --freezeParameters rgx{lumi.*},SF_Bin2,SF_Bin1,SF_Bin0,r_PassBin0,r_PassBin1,r_PassBin2,rgx{prop_bin.*} --verbose 3
combine -M MultiDimFit -d ${1} -m 1000 --algo singles --cl=0.68 --redefineSignalPOIs ${POI} -n scan --freezeParameters rgx{CMS.*},rgx{PS.*},rgx{.*ACCEPT},rgx{.*norm},rgx{lumi.*} 
