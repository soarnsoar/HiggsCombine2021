...
for Year in ${ARR_Year[@]};do
	...
	for Mass in ${ARR_Mass[@]};do
		...
		for sig in ${ARR_expectedSig[@]};do
		    ...
		    python ../../python_tool/ExportShellCondorSetup.py -c "combineTool.py -d ${Workspace} -M FitDiagnostics --plots -m ${Mass} -t -1 --expectSignal=${sig} --freezeParameters rgx{prop_.*},rgx{deltaTheory.*xsec},rgx{CMS.*},rgx{CMS_HEM1516.*},rgx{CMS_PU_.*},rgx{CMS_ak8jet.*},rgx{CMS_ak4jet.*},rgx{CMS_btag.*},rgx{CMS_eff.*},rgx{CMS_res.*},rgx{CMS_scale.*},rgx{.*norm.*},rgx{PS.*},rgx{QCDscale.*},rgx{lumi.*},rgx{mjjshape.*},rgx{pdf.*} --setParameterRanges r=0,10&&python ../../diffNuisances.py fitDiagnostics.Test.root -g outputfile.root" -d WORDIR_CONDOR_TEST3 -n ${WORKDIR} -m 1 -s 
		    cd -
