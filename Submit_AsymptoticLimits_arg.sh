...
for MASS in ${ARR_MASS[@]};do
    for CUT in ${ARR_CUT[@]};do	    
        python CondorSubmit_AsymptoticLimits.py -y ${YEAR} -m ${MASS} -b ${BST} --cut nocut -f ggfonly
	#python CondorSubmit_AsymptoticLimits.py  -y ${YEAR} -m ${MASS} -b ${BST} -f ggfonly -c ${CUT}
        if [[ $BST == "Boosted" ]];then
            MELACUT=_M900C0.01_$CUT
            python CondorSubmit_AsymptoticLimits.py -y ${YEAR} -m ${MASS} -b ${BST} --cut $MELACUT -f ggfonly
